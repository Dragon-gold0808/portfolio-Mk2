...
const ArticlesPost = ({
  ...
  // console.log("ddd = ",!!featured && visible);
...
export const Article = ({posts, featured, id, sectionRef, visible}) => {
  ...
  useEffect(() => {
    if(value >= 5) setFull(true);
  }, [value]);

  const postsHeader = (
    <header className={styles.header}>
      <Heading className={styles.heading} level={3} as="h1">
        <DecoderText text="Atricles" start = {visible}/>
      </Heading>
  ...
  const checkFunction = (item) => {
    ...
    return (String(item.title).toUpperCase().includes(searchText.value) || String(item.abstract).includes(searchText.value));
  };
  ...
  return (
    <article className={styles.articles} id = {id} ref = {sectionRef} onClick = {(e) => {
      // e.preventDefault();
